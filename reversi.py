...
class Reversi: 
    ...
    def valid_move(self, r, c, player = None):
        ...
        for d in range(8):
            ...
            if nr < 0 or nr >= 8 or nc < 0 or nc >= 8:
                continue
            if grid[nr][nc] == 0 or grid[nr][nc] == player:
                continue
            while nr >= 0 and nr < 8 and nc >= 0 and nc < 8 and grid[nr][nc] != 0:
    ...
    def make_move(self, r, c): # performs move, returns all swapped pieces
        ...
        for d in range(8):
            nr = r + dy[d]
            nc = c + dx[d]
            if nr < 0 or nr >= 8 or nc < 0 or nc >= 8 or grid[nr][nc] == 0 or grid[nr][nc] == player:
                continue
            valid_dir = False
            todo = []
            while nr >= 0 and nr < 8 and nc >= 0 and nc < 8 and grid[nr][nc] != 0:
    ...
    def game_over(self):
        return not len(self.get_moves(-1)) and not len(self.get_moves(1))

    def tie(self): 
        return self.count[-1] == self.count[1] and self.game_over()
    
    def win(self, player):
        return self.game_over() and self.count[player] > self.count[player * -1]
